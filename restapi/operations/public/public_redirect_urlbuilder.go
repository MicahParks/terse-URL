...
func (o *PublicRedirectURL) Build() (*url.URL, error) {
	...
	var _path = "/{shortenedURL}"

	shortenedURL := o.ShortenedURL
	if shortenedURL != "" {
		_path = strings.Replace(_path, "{shortenedURL}", shortenedURL, -1)
	} else {
		return nil, errors.New("shortenedUrl is required on PublicRedirectURL")
	}
...
func (o *PublicRedirectURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PublicRedirectURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PublicRedirectURL")
	}
