...
func (o *TerseVisitsURL) Build() (*url.URL, error) {
	...
	var _path = "/api/visits/{shortened}"

	shortened := o.Shortened
	if shortened != "" {
		_path = strings.Replace(_path, "{shortened}", shortened, -1)
	} else {
		return nil, errors.New("shortened is required on TerseVisitsURL")
	}
...
func (o *TerseVisitsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on TerseVisitsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on TerseVisitsURL")
	}
